	...
	public class Parser {
		...
		public Parser() {
			...
			nodes = new List<Computer> {
				ParseNode(Environment.GetFolderPath(Environment.SpecialFolder.Personal) + "/IntroExtension/Nodes/ExampleComputer.xml")
			};
		}

		private Computer ParseNode(string filename) {
		...
		private int PortNameToInt(string portName) {
		...
		private CustomThemeFile ParseCustomThemeFile(XmlNode fileXml) {
			string filePath = fileXml.Attributes["path"].Value;
			string fileName = fileXml.Attributes["name"].Value;
		...
		private EncryptedFile ParseEncryptedFile(XmlNode fileXml) {
			string filePath = fileXml.Attributes["path"].Value;
			string fileName = fileXml.Attributes["name"].Value;
		...
		private EOSDevice ParseEOSDevice(XmlNode node) {
		...
		private Mail ParseMail(XmlNode mail) {
			Mail m;
		...
		private MailServerDaemon ParseMailDaemon(XmlNode mailNode) {
		...
		private UploadServerDaemon ParseUploadDaemon(XmlNode uploadNode) {
		...
		private WebServerDaemon ParseWebServerDaemon(XmlNode webServerNode) {
		...
		private MessageBoardDaemon ParseMessageBoardDaemon(XmlNode daemon) {
		...
		private VariableMissionListingServer ParseVariableMissionListingServer(XmlNode daemon) {
		...
		private MissionHubServer ParseMissionHubServer(XmlNode daemon) {
		...
		private CreditsDaemon ParseCreditsDaemon(XmlNode daemon) {
			string title = daemon.Attributes["Title"].Value;
		...
		private MemoryDumpFile ParseMemDumpFile(XmlNode file) {
		...
		private Memory ParseMemory(XmlNode memory) {
		...
		private LogoDaemon ParseLogoDaemon(XmlNode node) {
		...
		private LogoCustomConnectDisplayDaemon ParseLCCDD(XmlNode node) {
		...
		private WhitelistAuthenticationDaemon ParseWhitelistAuthenticationDaemon(XmlNode node) {
		...
		private IRCDaemon ParseIRCDaemon(XmlNode node) {
		...
		private DHSDaemon ParseDHSDaemon(XmlNode node) {
